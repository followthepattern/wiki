...
If you don't define size to an array, Golang will handle it as a slice.
...
You can find the official doc [here](https://pkg.go.dev/golang.org/x/exp/slices), I will review couple of the functions and discuss them.
...
The `DeleteFunc` function zeroes the elements of the given slice, if the condition passed on the certain element. It doesn't change the capacity, only zeroes out the values.
